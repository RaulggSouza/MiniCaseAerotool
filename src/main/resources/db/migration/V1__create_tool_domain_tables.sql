...
CREATE TABLE tool_item(
    ...
    CONSTRAINT fk_tool_item_to_tool FOREIGN KEY(tool_id) REFERENCES tool(id),
    CONSTRAINT fk_tool_item_to_status FOREIGN KEY(status_id) REFERENCES tool_item_status(id),
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMPTZ
);
